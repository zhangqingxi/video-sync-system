...
class APIService:
    ...
    def fetch_video_list(self, page: int = 1) -> list[Video]:
        ...
        try:
            ...
            response: dict[str, Any] = self._http.post_with_auth(
                endpoint=self._api_config.endpoints.video_list, payload=params
            )
            ...
            self._logger.info(
                f"第 {page} 页获取到 {len(items)} 条基础数据（总记录数: {total}）"
            )
    ...
    def fetch_video_detail(self, video_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            response: dict[str, Any] = self._http.post_with_auth(
                endpoint=self._api_config.endpoints.video_detail, payload=params
            )
