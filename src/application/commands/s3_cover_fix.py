...
class S3CoverFixCommand(BaseCommand):
    """S3 Cover资源修复命令"""
    ...
    def description(self) -> str:
        return "修复S3上的cover图片资源"
    
    def execute(self) -> int:
        """执行修复"""
        try:
            self.logger.info("=" * 60)
            self.logger.info("开始S3 Cover资源修复")
            self.logger.info("=" * 60)
            ...
            # 获取失败ID列表
            failed_ids: list[int] = state_manager.get_s3_failed_cover_ids()
            ...
            with ThreadPoolExecutor(max_workers=thread_count) as executor:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if success:
                            success_count += 1
                            state_manager.remove_s3_failed_cover_id(video_id=video_id)
    ...
    def _fix_video_cover(
        ...
        try:
            ...
            # 上传到S3
            if not s3_adapter.upload_from_url(resource_url=cover_url, resource_type='cover', resource_key=cover_key):
