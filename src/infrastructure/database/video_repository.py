...
class VideoRepositoryImpl(VideoRepository):
    ...
    def __init__(
        self, db_pool: DatabasePool, config: DatabaseConfig, logger: LoggerProvider
    ):
        """
        初始化视频仓库
    ...
    def get_by_id(self, video_id: int) -> dict[str, Any] | None:
        ...
        """
        query: str = f"""
            SELECT * FROM `{self._table}` WHERE vod_douban_id = %s LIMIT 1
        """
        results: list[dict[str, Any]] = self._db_pool.execute_query(
            query=query, params=(video_id,)
        )
        return results[0] if results else None

    def get_videos_after_id(
        ...
        """
        query: str = f"""
            SELECT * FROM `{self._table}` WHERE vod_douban_id > %s ORDER BY vod_douban_id ASC LIMIT %s
        """
    ...
    def insert(self, vod_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            params: tuple = (
                ...
                vod_data["vod_total"],
                vod_data["vod_pic"],
                vod_data["vod_pic_thumb"],
                vod_data["vod_pic_slide"],
                vod_data["vod_lang"],
                vod_data["vod_year"],
                vod_data["vod_class"],
                ...
                vod_data["vod_time"],
                vod_data["vod_time_add"],
                vod_data["vod_down_url"],
                vod_data["vod_letter"],
                vod_data["vod_color"],
                vod_data["vod_pic_screenshot"],
                vod_data["vod_actor"],
                vod_data["vod_writer"],
                vod_data["vod_behind"],
                vod_data["vod_remarks"],
                vod_data["vod_pubdate"],
                vod_data["vod_serial"],
                vod_data["vod_status"],
                vod_data["vod_tag"],
                vod_data["vod_douban_id"],
                vod_data["vod_points"],
                vod_data["vod_points_play"],
                vod_data["vod_points_down"],
                vod_data["vod_trysee"],
                vod_data["vod_hits"],
            )
            ...
            self._logger.info(
                f"视频插入成功: '{vod_data['vod_name']}' (ID: {vod_data['vod_douban_id']})"
            )
            return True
    ...
    def exists(self, video_id: int) -> bool:
        ...
        """
        query: str = f"""
            SELECT COUNT(*) as count FROM `{self._table}` WHERE vod_douban_id = %s
        """
        results: list[dict[str, Any]] = self._db_pool.execute_query(
            query=query, params=(video_id,)
        )
    ...
    def get_all_video_ids(self) -> list[int]:
        ...
        """
        query: str = f"""
            SELECT vod_douban_id FROM `{self._table}` ORDER BY vod_douban_id ASC
        """
    ...
    def get_videos_batch(self, video_ids: list) -> list[dict[str, Any]]:
        ...
        query: str = f"""
            SELECT * FROM `{self._table}` WHERE vod_douban_id IN ({placeholders}) ORDER BY vod_douban_id ASC
        """
