...
class S3IndexCheckCommand(BaseCommand):
    """S3 Index资源检查命令"""

    @property
    def name(self) -> str:
        return "s3_index_check"

    @property
    def description(self) -> str:
        return "检查S3上的index m3u8资源是否存在"

    def execute(self) -> int:
        """执行检查"""
        try:
            self.logger.info("=" * 60)
            self.logger.info("开始S3 Index资源检查")
            self.logger.info("=" * 60)
            ...
            # 获取上次检查的ID
            last_checked_id: int = state_manager.get_last_checked_s3_index()
            ...
            if videos:
                ...
                state_manager.update_last_checked_s3_index(video_id=max_id)
