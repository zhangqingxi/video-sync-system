    ...
    def execute(self) -> int:
        ...
        try:
            ...
            while True:
                try:
                    # 1. 获取视频列表（仅基础信息）
                    videos: list[Video] = api_service.fetch_video_list(page=current_page)
                    
                    if not videos:
                        self.logger.warning(f"第 {current_page} 页无数据")
                        break
                    
                    processed_videos: list[dict] = []  # 存储处理成功的视频数据(vod_dict)用于站点同步
                    ...
                    for video in videos:
                        ...
                        # 3. 获取视频详情（仅当视频不存在时）
                        self.logger.info(f"获取视频详情: '{video.title}' (ID: {video.id})")
                        detail_data: dict[str, Any] = api_service.fetch_video_detail(video_id=video.id)
                        if not detail_data:
                            self.logger.warning(f"无法获取视频详情，跳过: ID={video.id}")
                            continue
                        ...
                        if video_repo.insert(vod_data=vod_dict):
                            ...
                            try:
                                ...
                                if uploaded:
                                    ...
                                    total_oss_failed += 1
                                    state_manager.add_oss_failed_origin_id(video_id=int(video.id))
                                    
                            except Exception as e:
                                self.logger.error(f"OSS上传异常: ID={video.id}, Error={e}")
                                total_oss_failed += 1
                                state_manager.add_oss_failed_origin_id(video_id=int(video.id))
                        else:
                    ...
                    if processed_videos and domains:
                        try:
                            ...
                            self.logger.info(f"开始同步 {len(processed_videos)} 个视频到站点")
                            ...
                            failed_by_domain: dict[str, list[int]] = site_service.sync_to_sites(videos=processed_videos)
                            ...
                            for domain, failed_ids in failed_by_domain.items():
                                ...
                                success_count: int = len(processed_videos) - len(failed_ids)
                            ...
                            for domain in domains:
                                for video_data in processed_videos:
                                    video_id: int = video_data.get('vod_douban_id')
                                    if video_id:
                                        state_manager.add_site_failed_id(domain=domain, video_id=video_id)
                                        total_site_failed += 1
