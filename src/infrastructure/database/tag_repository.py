...
class TagRepositoryImpl:
    ...
    def ensure_tag_exists(self, tag_name: str) -> int:
        ...
        try:
            # 先尝试查找
            query: str = f"SELECT tag_id FROM {self.tag_table} WHERE tag_name = %s"
            results: list[dict[str, Any]] = self.db_pool.execute_query(query=query, params=(tag_name,))
            ...
            # 不存在则插入
            insert_query: str = f"""
                INSERT INTO {self.tag_table} 
                (tag_name, tag_recommend) 
                VALUES (%s, 0)
            """
    ...
    def insert_video_tag(self, douban_id: int, tag_id: int, type_id: int) -> bool:
        ...
        try:
            query: str = f"""
                INSERT IGNORE INTO {self.video_tag_table} 
                (douban_id, tag_id, type_id) 
                VALUES (%s, %s, %s)
            """
            self.db_pool.execute_update(query=query, params=(douban_id, tag_id, type_id))
            return True
                
        except Exception as e:
            self.logger.error(f"插入视频标签失败: douban_id={douban_id}, tag_id={tag_id}, error={e}")
            return False
    ...
    def get_video_tag_count(self, douban_id: int) -> int:
        ...
        try:
            query: str = f"""
                ...
                FROM {self.video_tag_table} 
                WHERE douban_id = %s
            """
            results: list[dict[str, Any]] = self.db_pool.execute_query(query=query, params=(douban_id,))
    ...
    def delete_video_tags(self, douban_id: int) -> bool:
        ...
        try:
            query: str = f"""
                DELETE FROM {self.video_tag_table} 
                WHERE douban_id = %s
            """
