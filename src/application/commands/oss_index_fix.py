...
class OSSIndexFixCommand(BaseCommand):
    ...
    def execute(self) -> int:
        ...
        try:
            ...
            # 获取失败ID列表
            failed_ids: list[int] = state_manager.get_oss_failed_index_ids()
            ...
            with ThreadPoolExecutor(max_workers=thread_count) as executor:
                ...
                for video_id in failed_ids:
                    future: Future[bool] = executor.submit(
                        self._fix_video_index, video_id, oss_adapter, http_client
                    )
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if success:
                            success_count += 1
                            state_manager.remove_oss_failed_index_id(video_id=video_id)
    ...
    def _fix_video_index(
        self, video_id: int, oss_adapter: StorageProvider, http_client: HTTPClient
    ) -> bool:
        """
        修复单个视频的index资源
        ...
        try:
            # 尝试处理第1集
            return self._process_episode(video_id, 1, oss_adapter, http_client)
    ...
    def _process_episode(
        ...
        try:
            ...
            if not oss_adapter.upload_file(
                key=index_key,
