...
class OSSOriginFixCommand(BaseCommand):
    """OSS origin资源修复命令"""
    
    @property
    def name(self) -> str:
        return "oss_origin_fix"
    
    @property
    def description(self) -> str:
        return "同步失败的OSS origin m3u8资源"
    
    def execute(self) -> int:
        """执行同步"""
        try:
            self.logger.info("=" * 60)
            self.logger.info("开始OSS origin资源修复")
            self.logger.info("=" * 60)
    ...
    def _sync_video(
        ...
        try:
            # 1. 从API获取最新详情
            detail_data: dict[str, Any] = api_service.fetch_video_detail(video_id=str(video_id))
