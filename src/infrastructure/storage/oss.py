...
class OSSAdapter(BaseStorageAdapter):
    ...
    def __init__(
        ...
        try:
            # 创建凭证提供者（直接传参）
            credentials_provider: oss.credentials.CredentialsProvider = oss.credentials.StaticCredentialsProvider(
            ...
            self.logger.info(f"OSS客户端初始化成功: region={config.region}, bucket={config.bucket}")
