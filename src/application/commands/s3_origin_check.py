...
class S3OriginCheckCommand(BaseCommand):
    ...
    @property
    def name(self) -> str:
        return "s3_origin_check"

    @property
    def description(self) -> str:
        return "检查S3上的origin m3u8资源是否存在"

    def execute(self) -> int:
        """执行检查"""
        try:
            self.logger.info("=" * 60)
            self.logger.info("开始S3 Origin资源检查")
            self.logger.info("=" * 60)
            ...
            # 获取上次检查的ID
            last_checked_id: int = state_manager.get_last_checked_s3_origin()
            ...
            with ThreadPoolExecutor(max_workers=thread_count) as executor:
                ...
                for video_data in videos:
                    ...
                    future: Future[dict[int, list[int]]] = executor.submit(
                        self._check_video_origin, s3_adapter, video_id, video_episodes, video_origin
                    )
                    ...
                    try:
                        ...
                        if failed_episodes:
                            ...
                            for vid, episodes in failed_episodes.items():
                                failed_count += 1
                                state_manager.add_s3_failed_origin_episodes(
            ...
            if videos:
                ...
                state_manager.update_last_checked_s3_origin(video_id=max_id)
    ...
    def _check_video(
        self, s3_adapter: StorageProvider, video_id: int, video_episode: int, video_origin: str
    ) -> bool:
        """
        检查单个视频的单集index资源
