...
class SiteFixCommand(BaseCommand):
    ...
    def execute(self) -> int:
        ...
        try:
            ...
            failed_ids_by_domain: dict[str, list[int]] = (
                state_manager.get_site_failed_ids()
            )
            ...
            for domain, failed_ids in failed_ids_by_domain.items():
                ...
                self.logger.info(
                    f"[{domain}] 从数据库获取到 {len(videos_data)} 条视频数据"
                )
                ...
                # 批量同步到该域名
                failed_by_domain: dict[str, list[int]] = site_service.sync_to_sites(
                    videos=videos_data
                )
                ...
                self.logger.info(
                    f"[{domain}] 修复完成: 成功={success_count}, 仍失败={len(new_failed_ids)}"
                )
