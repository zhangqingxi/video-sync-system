...
class S3IndexFixCommand(BaseCommand):
    ...
    def execute(self) -> int:
        ...
        try:
            ...
            # 获取失败ID列表
            failed_data: dict[str, list[int]] = state_manager.get_s3_failed_index_episodes()
            ...
            with ThreadPoolExecutor(max_workers=thread_count) as executor:
                ...
                for video_data in videos:
                    ...
                    future: Future[dict[int, list[int]]] = executor.submit(
                        self._fix_video_index, video_id, video_episodes, video_origin, s3_adapter
                    )
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if not result:
                            # 所有集数都修复成功，移除该视频ID
                            success_count += 1
                            state_manager.remove_s3_failed_index_episodes(video_id=video_id)
                            self.logger.info(f"修复成功: ID={video_id}")
                        else:
                            # 还有失败的集数，更新失败列表
                            failed_count += 1
                            for vid, episodes in result.items():
                                state_manager.remove_s3_failed_index_episodes(
    ...
    def _fix_video_index(
        self, video_id: int, video_episodes: list[int], video_origin: str, s3_adapter: StorageProvider
    ) -> dict[int, list[int]]:
        """
        修复单个视频的index资源
    ...
    def _process_episode(
        ...
        try:
            ...
            if not s3_adapter.upload_file(content=index_content.encode("utf-8"), key=index_key, content_type=content_type):
