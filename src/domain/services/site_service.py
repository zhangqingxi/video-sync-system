...
class SiteService:
    ...
    def sync_to_sites(self, videos: list[dict[str, Any]]) -> dict[str, list[int]]:
        ...
        all_video_ids: set[int] = {
            int(v["vod_douban_id"]) for v in videos if v.get("vod_douban_id")
        }
        ...
        for domain in self.domains:
            try:
                ...
                if failed_ids:
                    self.logger.warning(
                        f"{domain} 同步部分失败，失败数量: {len(failed_ids)}"
                    )
                    failed_by_domain[domain] = [
                        int(video_id) for video_id in failed_ids
                    ]
                else:
