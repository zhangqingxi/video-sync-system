...
class BaseStorageAdapter(ABC):
    ...
    def upload_from_url(
        self, resource_url: str, resource_type: str, resource_key: str
    ) -> bool:
        """从URL下载并上传"""
        try:
            ...
            response: requests.Response = requests.get(
                url=resource_url, timeout=30, headers=headers, verify=False
            )
            ...
            if resource_type == "cover":
                ...
                content = self._keep_remote_ts_paths(
                    content=content, base_url=resource_url
                )
            elif resource_type == "index":
                content_type = "application/vnd.apple.mpegurl"

            return self.upload_file(
                key=resource_key, content=content, content_type=content_type
            )

        except Exception as e:
            self.logger.error(
                f"从URL上传失败: url={resource_url}, type={resource_type}, key={resource_key}, error={e}"
            )
            return False
    ...
    def generate_key(
        ...
        vod_hex: str = (
            vod_encrypted.replace("+", "-").replace("/", "_").replace("=", "")
        )
    ...
    def process_single_video_sync(
        ...
        try:
            self.logger.info(
                f"开始同步视频: {resource_title} (douban_id={resource_id}, type={resource_type})"
            )
            ...
            if resource_type in ("origin", "index"):
                ...
                for index, m3u8_url in enumerate(video_list, start=1):
                    ...
                    if not self.upload_from_url(
                        resource_url=m3u8_url,
                ...
                if not self.upload_from_url(
                    resource_url=cover, resource_type="cover", resource_key=cover_key
                ):
            ...
            self.logger.info(
                f"视频同步完成: {resource_title} (douban_id={resource_id}, type={resource_type})"
            )
            return True

        except Exception as e:
            self.logger.error(
                f"同步视频失败: {resource_title} (douban_id={resource_id}, type={resource_type}), error={e}"
            )
            return False
    ...
    def _keep_remote_ts_paths(content: bytes, base_url: str) -> bytes:
        ...
        for line in lines:
            ...
            if (
                ...
                and (stripped.endswith(".ts") or "/ts?" in stripped)
            ):
